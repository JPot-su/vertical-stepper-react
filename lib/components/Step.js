...
export default function Step({
            ...
            if (!isScrolledOnce) {
                if (isScrolled) {
                    try {
                        ...
                        await step.setSuccess(isScrolled);
                    } catch {
    ...
    }, [isScrolled, isScrolledOnce])

    useEffect(() => {
        if (success && typeof step.onSuccess === "function") {
            step.onSuccess();
        }
    }, [success])
    ...
    return (<>
        <div ref={elRef}>
            <StepLabel
                icon={isError ? step.errorIcon : success ? step.successIcon : step.icon}
                title={step.title}
            />
            <StepContent
                isError={isError}
